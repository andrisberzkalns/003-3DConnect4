...
import { useThree, MeshProps, MeshStandardMaterialProps } from '@react-three/fiber';
...
export const Piece = (props: JSX.IntrinsicElements['group'] & {pos: THREE.Vector3, square: ESquareState}) => {
    const { square } = props;
    ...
    const ESquareMaterialMap: {[key in ESquareState]: {mesh?: MeshProps, meshStandardMaterial?: MeshStandardMaterialProps}} = {
        [ESquareState.Empty]: {
            mesh: {
    ...
    return (
        <group ref={group} {...props} dispose={null} position={[positions.x, 0.29 + (props.pos.z * 0.27), positions.y]} rotation={[0,positions.x + positions.y + props.pos.z / 27 * 2 * Math.PI,0]}>
            <group name="Scene">
                <mesh name="Piece" geometry={nodes.Piece.geometry} position={[0, 14.256, 0]} rotation={[Math.PI / 2, 0, 0]} scale={0.015} castShadow receiveShadow {...ESquareMaterialMap[square].mesh}>
                {
                    ESquareMaterialMap[square].meshStandardMaterial !== undefined && <meshStandardMaterial attach="material" name="Glow" color="#ff0000" {...ESquareMaterialMap[square].meshStandardMaterial}/>
                }
                </mesh>
                <PositionalAudio 
                    autoplay
